...
class RayTracingMaterial extends Material {
...
async function buildRayTracingMaterial(vertexPath, fragmentPath) {


    let vertexShader = await getShaderString(vertexPath);
    let fragmentShader = await getShaderString(fragmentPath);

    return new RayTracingMaterial(vertexShader, fragmentShader);
